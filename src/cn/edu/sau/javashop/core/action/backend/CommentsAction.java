...
import cn.edu.sau.javashop.core.model.support.CommentDTO;
...
public class CommentsAction extends WWAction {
	...
	public String add() {
		...
		try {
			...
			System.out.println("Author: " + comments.getAuthor() + " Title:" + comments.getTitle() + " Comment:" + comments.getAcomment());
	...
	public String show() {
		try{
			...
			this.json = "{'result':0,'message':'操作成功'}";
			System.out.println("Title: " + comments.getTitle() + "Comment: " + comments.getAcomment());
		}catch(RuntimeException e){
			e.printStackTrace();
			this.json = "{'result':1,'message':'操作失败'}";
		}
